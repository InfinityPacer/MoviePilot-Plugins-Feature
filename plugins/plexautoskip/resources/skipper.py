...
from app.plugins.plexautoskip.resources.binge import BingeSessions
from app.plugins.plexautoskip.resources.customEntries import CustomEntries
from app.plugins.plexautoskip.resources.log import getLogger
from app.plugins.plexautoskip.resources.mediaWrapper import Media, MediaWrapper, PLAYINGKEY, STOPPEDKEY, PAUSEDKEY, \
    BUFFERINGKEY, DURATION_TOLERANCE, GRANDPARENTRATINGKEY, PARENTRATINGKEY, rd
from app.plugins.plexautoskip.resources.settings import Settings
from app.plugins.plexautoskip.resources.sslAlertListener import SSLAlertListener
...
class Skipper():
    ...
    def __init__(self, server: PlexServer, settings: Settings, logger: logging.Logger = None) -> None:
        ...
        self.log.debug("%s init with leftOffset %d rightOffset %d" % (
            self.__class__.__name__, self.settings.leftOffset, self.settings.rightOffset))
        ...
        self.log.debug("Skip first episode series %s" % (self.settings.skipS01E01))
        self.log.debug("Skip first episode season %s" % (self.settings.skipE01))
    ...
    def checkMedia(self, mediaWrapper: MediaWrapper) -> None:
        ...
        if mediaWrapper.skipnext and mediaWrapper.ended and (
                mediaWrapper.viewOffset >= rd(mediaWrapper.media.duration * DURATION_TOLERANCE)):
            self.log.info(
                "Found ended session %s that has reached the end of its duration %d with viewOffset %d with skip-next enabled, will skip to next" % (
                    mediaWrapper, mediaWrapper.media.duration, mediaWrapper.viewOffset))
            self.seekTo(mediaWrapper, mediaWrapper.media.duration)
        elif mediaWrapper.ended:
            self.log.debug("Session %s has been marked as ended with viewOffset %d and state %s, removing" % (
                mediaWrapper, mediaWrapper.viewOffset, mediaWrapper.state))
            self.removeSession(mediaWrapper)
    ...
    def checkMediaSkip(self, mediaWrapper: MediaWrapper, leftOffset: int, rightOffset: int) -> None:
        ...
        for marker in skipMarkers:
            if marker.start <= mediaWrapper.viewOffset < rd(marker.end):
                self.log.info("Found a custom marker for media %s with range %d-%d and viewOffset %d (%d)" % (
                    mediaWrapper, marker.start, marker.end, mediaWrapper.viewOffset, marker.key))
        ...
        if self.settings.skiplastchapter and mediaWrapper.lastchapter and (
                mediaWrapper.lastchapter.start / mediaWrapper.media.duration) > self.settings.skiplastchapter:
            if mediaWrapper.lastchapter and mediaWrapper.lastchapter.start <= mediaWrapper.viewOffset < rd(
                    mediaWrapper.lastchapter.end):
                self.log.info(
                    "Found a valid last chapter for media %s with range %d-%d and viewOffset %d with skip-last-chapter enabled" % (
                        mediaWrapper, mediaWrapper.lastchapter.start, mediaWrapper.lastchapter.end,
        ...
        for chapter in mediaWrapper.chapters:
            if chapter.start <= mediaWrapper.viewOffset < rd(chapter.end):
                self.log.info("Found skippable chapter %s for media %s with range %d-%d and viewOffset %d" % (
                    chapter.title, mediaWrapper, chapter.start, chapter.end, mediaWrapper.viewOffset))
        ...
        for marker in mediaWrapper.markers:
            ...
            if (start) <= mediaWrapper.viewOffset < rd(marker.end):
                self.log.info("Found skippable marker %s for media %s with range %d(+%d)-%d(+%d) and viewOffset %d" % (
                    marker.type, mediaWrapper, marker.start, lo, marker.end, ro, mediaWrapper.viewOffset))
    ...
    def checkMediaVolume(self, mediaWrapper: MediaWrapper, leftOffset: int, rightOffset: int) -> None:
        ...
        if not mediaWrapper.loweringVolume and shouldLower:
            self.log.info(
                "Moving from normal volume to low volume viewOffset %d which is a low volume area for media %s, lowering volume to %d" % (
                    mediaWrapper.viewOffset, mediaWrapper, self.settings.volumelow))
            self.setVolume(mediaWrapper, self.settings.volumelow, shouldLower)
            return
        elif mediaWrapper.loweringVolume and not shouldLower:
            self.log.info(
                "Moving from lower volume to normal volume viewOffset %d for media %s, raising volume to %d" % (
                    mediaWrapper.viewOffset, mediaWrapper, mediaWrapper.cachedVolume))
    ...
    def shouldLowerMediaVolume(self, mediaWrapper: MediaWrapper, leftOffset: int, rightOffset: int) -> bool:
        ...
        for marker in customVolumeMarkers:
            if marker.start <= mediaWrapper.viewOffset < marker.end:
                self.log.debug(
                    "Inside a custom marker for media %s with range %d-%d and viewOffset %d (%d), volume should be low" % (
                        mediaWrapper, marker.start, marker.end, mediaWrapper.viewOffset, marker.key))
                return True
        ...
        if self.settings.skiplastchapter and mediaWrapper.lastchapter and (
            ...
            if mediaWrapper.lastchapter and mediaWrapper.lastchapter.start <= mediaWrapper.viewOffset <= mediaWrapper.lastchapter.end:
                self.log.debug(
                    "Inside a valid last chapter for media %s with range %d-%d and viewOffset %d with skip-last-chapter enabled, volume should be low" % (
                        mediaWrapper, mediaWrapper.lastchapter.start, mediaWrapper.lastchapter.end,
                        mediaWrapper.viewOffset))
                return True
        ...
        for chapter in mediaWrapper.chapters:
            if chapter.start <= mediaWrapper.viewOffset < chapter.end:
                self.log.debug(
                    "Inside chapter %s for media %s with range %d-%d and viewOffset %d, volume should be low" % (
                        chapter.title, mediaWrapper, chapter.start, chapter.end, mediaWrapper.viewOffset))
                return True
        ...
        for marker in mediaWrapper.markers:
            ...
            if (marker.start + lo) <= mediaWrapper.viewOffset < (marker.end + ro):
                self.log.debug(
                    "Inside marker %s for media %s with range %d(+%d)-%d(+%d) and viewOffset %d, volume should be low" % (
                        marker.type, mediaWrapper, marker.start, lo, marker.end, ro, mediaWrapper.viewOffset))
                return True
        return False
    ...
    def _seekTo(self, mediaWrapper: MediaWrapper, targetOffset: int) -> None:
        try:
            ...
            self.log.debug(
                "TimeoutError, removing from cache to prevent false triggers, will be restored with next sync")
            self.removeSession(mediaWrapper)
        except:
            self.log.exception(
                "Exception, removing from cache to prevent false triggers, will be restored with next sync")
            self.removeSession(mediaWrapper)

    def seekPlayerTo(self, player: PlexClient, mediaWrapper: MediaWrapper, targetOffset: int, pq: PlayQueue = None,
                     server: PlexServer = None) -> bool:
        if not player:
            return False
        ...
        try:
            try:
                if mediaWrapper.skipnext and targetOffset >= mediaWrapper.media.duration:
                    ...
                    if mediaWrapper.media.duration and targetOffset >= (
                            mediaWrapper.media.duration - self.CREDIT_SKIP_FIX.get(player.product, 0)):
                        self.log.debug(
                            "TargetOffset %d is greater or equal to duration of media %d(-%d), adjusting to match" % (
                    ...
                    if targetOffset <= mediaWrapper.viewOffset:
                        self.log.debug("TargetOffset %d is less than or equal to current viewOffset %d, ignoring" % (
                            targetOffset, mediaWrapper.viewOffset))
                        return False

                    self.log.info("Seeking %s player playing %s from %d to %d" % (
                        player.product, mediaWrapper, mediaWrapper.viewOffset, targetOffset))
    ...
    def skipPlayerTo(self, player: PlexClient, mediaWrapper: MediaWrapper, pq: PlayQueue, server: PlexServer) -> bool:
        ...
        if not pq:
            try:
                ...
                self.log.exception("")
                self.log.debug(
                    "Seek target is the end but unable to get existing PlayQueue %d (%s) data from server" % (
                ...
                if mediaWrapper.media.type == "episode":
                    ...
                    try:
                        ...
                        if episodes and episodes[-1] != mediaWrapper.media:
                            ...
                            self.log.debug("New queue contains %d items, selecting %s with index %s" % (
                                len(episodes), startItem, startItemIndex))
        ...
        try:
            try:
                ...
                if player.timeline and player.timeline.volume is not None:
                    ...
                    self.log.debug(
                        "Unable to access timeline data for player %s to cache previous volume value, will restore to %d" % (
                            player.product, previousVolume))
                self.log.info("Setting %s player volume playing %s from %d to %d" % (
                    player.product, mediaWrapper, previousVolume, volume))
    ...
    def validPlayer(self, player: PlexClient) -> bool:
        ...
        if bad and player.version and parse_version(self.safeVersion(player.version)) >= parse_version(bad):
            self.log.error(
                "Bad %s version %s due to Plex team removing 'Advertise as Player/Plex Companion' functionality. Please visit %s#notice to review this issue and voice your support on the Plex forums for this feature to be restored" % (
                    player.product, player.version, self.TROUBLESHOOT_URL))
            return False
                ...
                if pasIdentifier not in self.media_sessions:
                    ...
                    if self.verbose:
                        if mediaSession and mediaSession.session and mediaSession.player:
                            self.log.debug(
                                "Alert for %s with state %s viewOffset %d playQueueID %d location %s user %s player IP %s" % (
                                    pasIdentifier, state, viewOffset, playQueueID, mediaSession.session.location,
                                    mediaSession._username, mediaSession.player.address))
                        elif mediaSession and mediaSession.session:
                            self.log.debug(
                                "Alert for %s with state %s viewOffset %d playQueueID %d location %s user %s" % (
                                    pasIdentifier, state, viewOffset, playQueueID, mediaSession.session.location,
                                    mediaSession._username))
                        else:
                            self.log.debug(
                                "Alert for %s with state %s viewOffset %d playQueueID %d but no session data" % (
                                    pasIdentifier, state, viewOffset, playQueueID))
                    if mediaSession and mediaSession.session and mediaSession.session.location == 'lan':
                        wrapper = MediaWrapper(mediaSession, clientIdentifier, state, playQueueID, self.server,
                                               settings=self.settings, custom=self.customEntries, logger=self.log)
                    ...
                    if not mediaSession.ended and state in [STOPPEDKEY, PAUSEDKEY] and not self.getMediaSession(
                            sessionKey):
    ...
    def blockedClientUser(self, mediaWrapper: MediaWrapper) -> bool:
        ...
        # Clients/players
        if self.customEntries.allowedClients and (
                mediaWrapper.player.title not in self.customEntries.allowedClients and mediaWrapper.clientIdentifier not in self.customEntries.allowedClients):
            self.log.debug("Blocking %s based on no allowed player %s %s" % (
                mediaWrapper, mediaWrapper.player.title, mediaWrapper.clientIdentifier))
            return True
        elif self.customEntries.allowedClients:
            self.log.debug("Allowing %s based on allowed player %s %s" % (
                mediaWrapper, mediaWrapper.player.title, mediaWrapper.clientIdentifier))
        if self.customEntries.blockedClients and (
                mediaWrapper.player.title in self.customEntries.blockedClients or mediaWrapper.clientIdentifier in self.customEntries.blockedClients):
            self.log.debug("Blocking %s based on blocked player %s %s" % (
                mediaWrapper, mediaWrapper.player.title, mediaWrapper.clientIdentifier))
            return True
        return False
    ...
    def shouldAdd(self, mediaWrapper: MediaWrapper) -> bool:
        ...
        if mediaWrapper.media.type not in self.settings.types:
            self.log.debug("Blocking %s of type %s as its not on the approved type list %s" % (
                mediaWrapper, media.type, self.settings.types))
            return False

        if media.librarySectionTitle and media.librarySectionTitle.lower() in self.settings.ignoredlibraries:
            self.log.debug("Blocking %s in library %s as its library is on the ignored list %s" % (
                mediaWrapper, media.librarySectionTitle, self.settings.ignoredlibraries))
            return False
        ...
        if not self.settings.skipunwatched and not media.isWatched:
            self.log.debug(
                "Blocking %s, unwatched and skip-unwatched is %s" % (mediaWrapper, self.settings.skipunwatched))
            return False
        return True
        ...
        if hasattr(media, "episodeNumber"):
            if media.episodeNumber == 1:
                if self.settings.skipE01 == Settings.SKIP_TYPES.NEVER:
                    self.log.debug("Erasing tags %s, first episode in season and skip-first-episode-season is %s" % (
                        mediaWrapper, self.settings.skipE01))
                    ...
                    self.log.debug(
                        "Erasing tags %s, first episode in season and skip-first-episode-season is %s and isWatched %s" % (
                            mediaWrapper, self.settings.skipE01, media.isWatched))
                    mediaWrapper.tags = [t for t in mediaWrapper.tags if t in self.settings.firstsafetags]
                    mediaWrapper.updateMarkers()
            if hasattr(media, "seasonNumber") and media.seasonNumber == 1 and media.episodeNumber == 1:
                if self.settings.skipS01E01 == Settings.SKIP_TYPES.NEVER:
                    self.log.debug("Erasing tags %s, first episode in series and skip-first-episode-series is %s" % (
                        mediaWrapper, self.settings.skipS01E01))
                    mediaWrapper.tags = [t for t in mediaWrapper.tags if t in self.settings.firstsafetags]
                    mediaWrapper.updateMarkers()
                elif self.settings.skipS01E01 == Settings.SKIP_TYPES.WATCHED and not media.isWatched:
                    self.log.debug(
                        "Erasing tags %s, first episode in series and skip-first-episode-series is %s and isWatched %s" % (
                            mediaWrapper, self.settings.skipS01E01, media.isWatched))
                    mediaWrapper.tags = [t for t in mediaWrapper.tags if t in self.settings.firstsafetags]
                    mediaWrapper.updateMarkers()
        ...
        if hasattr(media, "episodeNumber") and hasattr(media, "seasonNumber"):
            ...
            if media.episodeNumber == curr_season.episodes()[-1].episodeNumber:
                if self.settings.skiplastepisodeseason == Settings.SKIP_TYPES.NEVER:
                    self.log.debug("Erasing tags %s, last episode in season and skip-last-episode-season is %s" % (
                        mediaWrapper, self.settings.skiplastepisodeseason))
                    ...
                    self.log.debug(
                        "Erasing tags %s, last episode in season and skip-last-episode-season is %s and isWatched %s" % (
                            mediaWrapper, self.settings.skiplastepisodeseason, media.isWatched))
                    mediaWrapper.tags = [t for t in mediaWrapper.tags if t in self.settings.lastsafetags]
                    mediaWrapper.updateMarkers()
            if media.seasonNumber == last_season.seasonNumber and media.episodeNumber == last_season.episodes()[
                -1].episodeNumber:
                if self.settings.skiplastepisodeseries == Settings.SKIP_TYPES.NEVER:
                    self.log.debug("Erasing tags %s, last episode in series and skip-last-episode-series is %s" % (
                        mediaWrapper, self.settings.skiplastepisodeseries))
                    mediaWrapper.tags = [t for t in mediaWrapper.tags if t in self.settings.lastsafetags]
                    mediaWrapper.updateMarkers()
                elif self.settings.skiplastepisodeseries == Settings.SKIP_TYPES.WATCHED and not media.isWatched:
                    self.log.debug(
                        "Erasing tags %s, last episode in series and skip-last-episode-series is %s and isWatched %s" % (
                            mediaWrapper, self.settings.skiplastepisodeseries, media.isWatched))
                    mediaWrapper.tags = [t for t in mediaWrapper.tags if t in self.settings.lastsafetags]
                    mediaWrapper.updateMarkers()
    ...
    def addSession(self, mediaWrapper: MediaWrapper) -> None:
        if mediaWrapper.customOnly:
            self.log.info(
                "Found blocked session %s viewOffset %d %s on %s (proxying: %s), using custom markers only, sessions: %d" % (
                    mediaWrapper, mediaWrapper.plexsession.viewOffset, mediaWrapper.plexsession._username,
                    mediaWrapper.player.product, mediaWrapper.player._proxyThroughServer, len(self.media_sessions)))
        else:
            self.log.info("Found new session %s viewOffset %d %s on %s (proxying: %s), sessions: %d" % (
                mediaWrapper, mediaWrapper.plexsession.viewOffset, mediaWrapper.plexsession._username,
                mediaWrapper.player.product, mediaWrapper.player._proxyThroughServer, len(self.media_sessions)))
    ...
    def ignoreSession(self, mediaWrapper: MediaWrapper) -> None:
        ...
        self.log.debug("Ignoring session %s %s, ignored: %d" % (
            mediaWrapper, mediaWrapper.plexsession._username, len(self.ignored)))
    ...
    def purgeOldSessions(self, mediaWrapper: MediaWrapper) -> None:
        for sessionMediaWrapper in list(self.media_sessions.values()):
            if sessionMediaWrapper.clientIdentifier == mediaWrapper.player.machineIdentifier:
                self.log.info("Session %s shares player (%s) with new session %s, deleting old session %s" % (
                    sessionMediaWrapper, mediaWrapper.player.machineIdentifier, mediaWrapper,
                    sessionMediaWrapper.plexsession.sessionKey))
