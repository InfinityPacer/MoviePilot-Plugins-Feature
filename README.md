...
- [ ] **动态删除种子（实验性功能）规则调整**：进一步评估用户设置的条件~~涉及上传资源权重较高的相关条件前置调整（平均上传速度）~~，即便没有触发删除阈值，也按对应的删种条件执行删除操作，即如果用户配置了删除条件，则每次刷流检查服务都会按用户设置的条件执行删除操作（排除H&R种子），然后再执行动态删除逻辑
  - [x] **规则调整**：「下载超时时间」已经前置调整，待进一步评估
...
![](image/2024-04-19-00-46-10.png)

### 3. 插件热加载

- 直接在Docker中调试插件时，无需重启容器即可完成插件热加载

![](image/2024-04-19-00-47-33.png)
